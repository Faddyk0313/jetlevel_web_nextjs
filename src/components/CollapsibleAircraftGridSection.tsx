...
const CollapsibleAircraftGridSection: React.FC<CollapsibleAircraftGridSectionProps> = ({
  title,
  content,
  isDefaultOpen = false,
}) => {
  ...
  return (
    <section className="border-b py-5">
      ...
      >
        ...
        {/* Grid Section */}
        <div id="aircraftGrid" className="grid sm:grid-cols-[repeat(auto-fit,_minmax(33%,_1fr))]">
          {content.map((item, index) => (
            <div key={index} className="p-4 py-2">
              <div className="text-sm">{item.fields.heading.text}</div>
              <div className="text-sm text-blue">{item.fields.value.text}</div>
            </div>
          ))}
        </div>
