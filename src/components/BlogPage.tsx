...
const BlogPage = ({ fields, date }: BlogPageProps) => {
    ...
    return (
        <>
            <section className='px-5 md:px-10 xl:px-20 pt-10 max-w-[1800px] mx-auto '>
            ...
            <section className="flex flex-col lg:flex-row gap-10 px-5 md:px-10 xl:px-20 py-7 max-w-[1800px] mx-auto">
                <div className="min-w-full md:min-w-[72%]">
                    <Breadcrumb />
                    <Image className='my-9' src={fields.hero_image.assets[0].asset.url} width={fields.hero_image.assets[0].asset.width} height={fields.hero_image.assets[0].asset.height} alt={fields.hero_image.assets[0].asset.name} />
                    {
                        fields.sections.blocks.map((section: any, index: number) => (
                            <div key={index} className='pb-8'>
                                <h2 className='pb-4 '>{section.fields.title.text}</h2>
                                <Markdown
                                    options={{
                                        overrides: {
                                            a: {
                                ...
                                >
                                    {section.fields.paragraph.text as string}
                                </Markdown>
                            </div>
                        ))
                    }
            ...
            <section id='next-post' className="relative object-cover overflow-hidden text-center group">
                <Image
                    className="object-cover w-full h-auto min-h-72 left-0 sm:-top-[60px] md:-top-[140px] lg:-top-[220px] xl:-top-[300px] opacity-100 absolute group-hover:scale-110 transition-all duration-[0.4s] ease-[cubic-bezier(0.05,0.2,0.1,1)] delay-[0s] -z-10"
                ...
                />
                <div className="flex flex-col justify-center items-center h-72 border-l border-l-white transition-all duration-[0.4s] ease-[cubic-bezier(0.05,0.2,0.1,1)] delay-[0s] z-10">
                    <span
